...
import type { TimestepData, TimestepState, TimestepStore } from '@/modules/canvas/presentation/types/stores/editor/timesteps';
...
export const createTimestepSlice: StateCreator<any, [], [], TimestepStore> = (set, get) => ({
    ...
    loadModels: async (preloadBehavior, onProgress, maxFramesToPreload, currentFrameIndex) => {
        ...
        const { loadGLB } = await import('@/modules/canvas/presentation/utilities/loader');
        ...
        const promises = targetTimesteps.map(async (timestep) => {
