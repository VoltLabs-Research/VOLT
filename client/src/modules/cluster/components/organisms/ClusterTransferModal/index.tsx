        ...
        >
            <Container className='d-flex column gap-1 p-1-5'>
                <Container className='d-flex column gap-05'>
                    <Title className='font-size-2 font-weight-5 color-secondary'>Destination cluster</Title>
                ...
                {queuedMessage && (
                    <Paragraph className='font-size-2 color-success'>{queuedMessage}</Paragraph>
                )}
                {error && (
                    <Paragraph className='font-size-2 color-danger'>{error}</Paragraph>
                )}
                <Container className='d-flex column gap-075'>
                    <Title className='font-size-2 font-weight-5 color-secondary'>Recent transfer jobs</Title>
                    {transferJobsQuery.isLoading ? (
                        <Paragraph className='font-size-1 color-muted'>Loading transfer jobs...</Paragraph>
                    ) : transferJobs.length > 0 ? (
                        transferJobs.map((job) => (
                            <Container key={job._id} className='d-flex column gap-05 p-1 radius-md bg-page'>
                                ...
                                {job.errorMessage && (
                                    <Paragraph className='font-size-1 color-danger'>{job.errorMessage}</Paragraph>
                                )}
                            </Container>
                        ))
                    ) : (
                        <Paragraph className='font-size-1 color-muted'>No transfer jobs have been requested for this cluster yet.</Paragraph>
                    )}
                </Container>
            </Container>
        </Modal>
