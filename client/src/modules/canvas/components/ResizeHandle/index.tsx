...
const ResizeHandle = ({ direction, isDragging, onPointerDown, onKeyDown, onDoubleClick, label, controls, valueMin, valueMax, valueNow }: ResizeHandleProps) => (
    <div className={`volt-container canvas-resize-handle z-5 canvas-resize-handle--${direction}${isDragging ? ' canvas-resize-handle--active' : ''}`} onPointerDown={onPointerDown} onKeyDown={onKeyDown} onDoubleClick={onDoubleClick} role="separator" tabIndex={0} aria-label={`${label} (double-click to reset)`} aria-controls={controls} aria-orientation={direction} aria-valuemin={valueMin} aria-valuemax={valueMax} aria-valuenow={valueNow} title='Double-click to reset'>
