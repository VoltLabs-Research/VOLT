            ...
            <div className='m-0 border-l-0 p-0' role='group' aria-label={`${analysis.pluginDisplayName} execution timeline`}>
                {rows.map((row) => (
                    <div key={row.key} className={cn(
                        ...
                        {row.cacheHit && <span className='invisible flex-none text-2xs leading-none text-muted opacity-0 transition-[opacity,visibility] duration-[120ms] ease-out group-hover:visible group-hover:opacity-100 group-focus-within:visible group-focus-within:opacity-100 max-md:hidden px-1 py-0.5'>cached</span>}
                        {row.durationMs !== undefined && (
                            <span className='invisible flex-none text-2xs leading-none text-muted opacity-0 transition-[opacity,visibility] duration-[120ms] ease-out group-hover:visible group-hover:opacity-100 group-focus-within:visible group-focus-within:opacity-100 max-md:hidden'>{formatDuration(row.durationMs)}</span>
                        )}
                    </div>
                ))}
            </div>
