...
export const MaybeContextMenu = ({ enabled, id, options, children }: MaybeContextMenuProps) => {
    ...
    return <ContextMenuPopover id={id} trigger={<Container>{children}</Container>} options={options} size='sm' />;
};
    ...
    return (
        <>
            {Array.from({ length: count }).map((_, i) => (
                <Container key={`canvas-tree-skel-${i}`} className={`canvas-tree-item d-flex items-center gap-05 color-secondary ${INDENT_CLASSES[indent]}`}>
                    <span className="canvas-tree-spacer" />
                    <Container className={`canvas-tree-skeleton${compact ? ' canvas-tree-skeleton--compact' : ''}`} />
                </Container>
            ))}
        </>
    );
...
export const CanvasTreeEmptyRow = ({ label, indent = 'base' }: CanvasTreeEmptyRowProps) => (
    <Container className={`canvas-tree-item d-flex items-center gap-05 color-secondary ${INDENT_CLASSES[indent]}`}>
