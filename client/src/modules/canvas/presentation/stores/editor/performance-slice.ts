...
export const createPerformanceSlice: StateCreator<any, [], [], PerformanceSlice> = (set, get) => ({
    performanceSettings: {
        ...
        setPreset: (preset: PerformancePreset) => set((s: PerformanceSlice) => ({ performanceSettings: { ...s.performanceSettings, ...cleanPowerPreference(presets[preset]) } })),

        setDpr: (partial: Partial<DprSettings>) => set((s: PerformanceSlice) => ({
            performanceSettings: { ...s.performanceSettings, dpr: { ...s.performanceSettings.dpr, ...partial } }
        })),

        setCanvas: (partial: Partial<CanvasSettings>) => set((s: PerformanceSlice) => ({
            performanceSettings: { ...s.performanceSettings, canvas: { ...s.performanceSettings.canvas, ...partial } }
        })),

        setPerformance: (partial: Partial<CanvasPerformanceProp>) => set((s: PerformanceSlice) => ({
            performanceSettings: { ...s.performanceSettings, performance: { ...s.performanceSettings.performance, ...partial } }
        })),

        setAdaptiveEvents: (partial: Partial<AdaptiveEventsSettings>) => set((s: PerformanceSlice) => ({
            performanceSettings: { ...s.performanceSettings, adaptiveEvents: { ...s.performanceSettings.adaptiveEvents, ...partial } }
        })),

        setInteractionDegrade: (partial: Partial<InteractionDegradeSettings>) => set((s: PerformanceSlice) => ({
            performanceSettings: { ...s.performanceSettings, interactionDegrade: { ...s.performanceSettings.interactionDegrade, ...partial } }
        })),

        reset: () => set((s: PerformanceSlice) => ({ performanceSettings: { ...s.performanceSettings, ...cleanPowerPreference(initial) } })),
