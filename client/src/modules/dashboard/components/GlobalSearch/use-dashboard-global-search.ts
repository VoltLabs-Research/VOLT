...
import useGlobalSearchKeyboardNavigation from './use-global-search-keyboard-navigation';
...
const useDashboardGlobalSearch = () => {
    ...
    const isSearchableQuery = trimmedQuery.length >= MIN_SEARCH_QUERY_LENGTH;
    const debouncedInput = useDebouncedValue(isSearchableQuery ? trimmedQuery : '', SEARCH_DEBOUNCE_MS);
    const debouncedQuery = isSearchableQuery ? debouncedInput : '';
    ...
    useEffect(() => {
        setShowResults(isSearchableQuery);
    }, [isSearchableQuery]);
