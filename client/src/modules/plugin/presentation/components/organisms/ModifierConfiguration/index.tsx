...
const ModifierConfiguration = ({
    ...
    }, [trajectoryId, trajectory, refetchTrajectory]);
    ...
    const availableArguments = useMemo(() => {
        return getAvailableArguments(modifierId);
    ...
    useEffect(() => {
        if (!hasAutoStarted.current && !isLoading && configFields.length === 0 && trajectoryId) {
    ...
    }, [configFields.length, isLoading, trajectoryId, modifierId, onAnalysisStart, onAnalysisSuccess, onAnalysisError, pluginRepository, config, selectedFrameOnly, currentTimestep]);
    ...
    return (
        <Container className={`modifier-configuration d-flex column p-1 ${className}`}>
