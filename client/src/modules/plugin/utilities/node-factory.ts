...
import { v4 } from 'uuid';
import { Exporter, ModifierContext, NodeType } from '../api/entities/workflow-enums';
import type { INodeData } from '../api/entities/workflow';

export const createNode = (type: NodeType, position: { x: number; y: number }): Node<INodeData> => {
    const id = v4();
