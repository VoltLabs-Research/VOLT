...
const KNOWN_HTTP_STATUSES = new Set([400, 401, 403, 404, 409, 422, 429, 500, 502, 503, 504]);

let clientTraceId: string | null = null;

const getClientTraceId = (): string => {
    if (!clientTraceId) {
        clientTraceId = `${Date.now()}-${Math.random().toString(16).slice(2)}`;
    }
...
export const createInstrumentedHttpClient = ({
    baseUrl,
    credential,
    timeout
}: CreateInstrumentedHttpClientOptions): HttpClient => {
    ...
    return {
        async request<T>(request: HttpRequest): Promise<T> {
            ...
            try {
                ...
                const fallbackCode = KNOWN_HTTP_STATUSES.has(status) ? `Http::${status}` : 'Internal::Server::Error';
                throw new ApiError(codeFromServer ?? fallbackCode, status, error);
            }
        }
    };
};
