...
class ErrorBoundary extends Component<ErrorBoundaryProps, ErrorBoundaryState> {
    ...
    render() {
        if (this.state.hasError) {
            return (
                <Container className='d-flex items-center content-center w-max h-max p-2'>
                    <Container className='d-flex column items-center text-center gap-1-5' role='alert' aria-live='assertive'>
                        ...
                        <Container className='d-flex column gap-05 items-center'>
                            ...
                            {this.state.error?.message && (
                                <Paragraph className='font-size-1 color-muted line-height-5'>
                                    {this.state.error.message}
                                </Paragraph>
                            )}
                        </Container>
