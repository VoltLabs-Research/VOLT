...
export default class BaseRepository{
    ...
    protected async getAllPaginated<T, P extends object = Record<string, unknown>>(
        path: string,
        params?: P
    ): Promise<PaginatedResponse<T>>{
        const raw = await this.client.get<RawPaginatedResponse<T>>(path, params as Record<string, unknown>);
