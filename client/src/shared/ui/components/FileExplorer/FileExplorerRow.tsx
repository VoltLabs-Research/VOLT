...
const FileExplorerRow = ({
    ...
    const content = (
        <>
            ...
            <span className={cn(ROW_META_CLASS_NAMES, "max-[900px]:before:content-['Type']")} title={type} aria-label={`Type: ${type ?? 'Not available'}`}>{type ?? '-'}</span>
            <span className={cn(ROW_META_CLASS_NAMES, "max-[900px]:before:content-['Size']")} title={size} aria-label={`Size: ${size ?? 'Not available'}`}>{size ?? '-'}</span>
            <span className={cn(ROW_META_CLASS_NAMES, "max-[900px]:before:content-['Date']")} title={date} aria-label={`Date: ${date ?? 'Not available'}`}>{date ?? '-'}</span>
        </>
    );
