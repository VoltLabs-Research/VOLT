...
export default class DailyActivityEvents {
    ...
    async analysisDeleted({ teamId, userId, pluginDisplayName }: EventMap['analysis.deleted']) {
        await dailyActivityService.recordActivity(teamId, userId, ActivityType.AnalysisDeletion, `Deleted analysis "${pluginDisplayName}"`);
    }
    ...
    async pluginExecutionRequested({ teamId, userId, pluginName, trajectoryName }: EventMap['PluginExecutionRequest']) {
        await dailyActivityService.recordActivity(teamId, userId, ActivityType.AnalysisPerformed, `started analysis on ${pluginName} for trajectory ${trajectoryName}`);
    }

    @Event('team-role.created')
    async teamRoleCreated({ teamId, userId, name }: EventMap['team-role.created']) {
        await dailyActivityService.recordActivity(teamId, userId, ActivityType.RoleCreation, `Created role "${name}"`);
    }

    @Event('team-role.deleted')
    async teamRoleDeleted({ teamId, userId, roleName }: EventMap['team-role.deleted']) {
        await dailyActivityService.recordActivity(teamId, userId, ActivityType.RoleDeletion, `Deleted role "${roleName}"`);
    }

    @Event('secret-key.created')
    async secretKeyCreated({ teamId, userId, name }: EventMap['secret-key.created']) {
        await dailyActivityService.recordActivity(teamId, userId, ActivityType.SecretKeyCreation, `Created secret key "${name}"`);
    }

    @Event('secret-key.deleted')
    async secretKeyDeleted({ teamId, userId, secretKeyName }: EventMap['secret-key.deleted']) {
        await dailyActivityService.recordActivity(teamId, userId, ActivityType.SecretKeyDeletion, `Deleted secret key "${secretKeyName}"`);
    }

    @Event('trajectory.created')
    async trajectoryCreated({ teamId, userId, trajectoryName }: EventMap['trajectory.created']) {
        await dailyActivityService.recordActivity(teamId, userId, ActivityType.TrajectoryUpload, `Uploaded trajectory "${trajectoryName}"`);
    }

    @Event('trajectory.deleted')
    async trajectoryDeleted({ teamId, userId, trajectoryName }: EventMap['trajectory.deleted']) {
        await dailyActivityService.recordActivity(teamId, userId, ActivityType.TrajectoryDeletion, `Deleted trajectory "${trajectoryName}"`);
    }

    @Event('whiteboard.created')
    async whiteboardCreated({ teamId, userId, whiteboardTitle }: EventMap['whiteboard.created']) {
        await dailyActivityService.recordActivity(teamId, userId, ActivityType.WhiteboardCreation, `Created whiteboard "${whiteboardTitle}"`);
    }

    @Event('whiteboard.deleted')
    async whiteboardDeleted({ teamId, userId, whiteboardTitle }: EventMap['whiteboard.deleted']) {
        await dailyActivityService.recordActivity(teamId, userId, ActivityType.WhiteboardDeletion, `Deleted whiteboard "${whiteboardTitle}"`);
    }
    ...
    async userActivityRecorded({ teamId, userId, minutes }: EventMap['user-activity.recorded']) {
        try {
            await dailyActivityService.recordOnlineMinutes(teamId, userId, minutes);
        } catch (error) {
