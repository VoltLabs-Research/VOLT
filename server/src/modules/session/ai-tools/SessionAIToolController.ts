...
export default class SessionAIToolController extends AIToolController {
    ...
    async manageSessions(input: ManageSessionsInput & AIToolScope) {
        switch (input.action) {
            case 'list': {
                const value = await sessionService.getActiveSessions(input.userId);
                return {
            ...
            case 'revoke': {
                ...
                await sessionService.revokeSession(input.sessionId, input.userId);
                return {
            ...
            case 'revoke_others': {
                const { revokedCount } = await sessionService.revokeAllSessions(input.userId, '');
                return {
