...
export default class ChatSocketPresenceService {
    ...
    async getUsersPresence(userIds: string[]): Promise<Record<string, ChatPresenceStatus>> {
        ...
        for (let index = 0; index < userIds.length; index++) {
            ...
            const socketIds = Array.isArray(redisResult)
                ? redisResult.filter((value): value is string => typeof value === 'string')
                : [];
        ...
        for (const userId of userIds) {
            ...
            presence[userId] = onlineSocketIds.length > 0 ? 'online' : 'offline';
        }
    ...
    private startPresenceRefresh(userId: string, socketId: string): void {
        const interval = setInterval(() => {
            void this.registerPresence(userId, socketId).catch((error) => {
