...
/**
 * Notebook POLICY over the generic container runtime.
 *
 * A notebook is just a `volt.managed` container that the DaemonExposureRegistry
 * discovers (by labels), readiness-gates (HTTP probe on the Jupyter API), and
 * publishes as an exposure — exactly like any other container. The cloud reaches
 * it through that exposure + the shared reverse-WS relay; there is no
 * notebook-specific addressing, host-port reservation, readiness origin
 * resolution, or `notebook.runtime.get` RPC anymore.
 *
 * What remains here is the irreducible notebook policy:
