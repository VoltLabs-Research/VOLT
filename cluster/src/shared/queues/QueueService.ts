...
import { QueueWorker } from '@shared/queues/QueueWorker';
import { getQueueNotifier } from '@shared/queues/QueueNotifier';
import { readPositiveIntegerEnv } from '@shared/utilities/env';
import { getQueueJobStore } from '@shared/queues/queue-job-store';
import type { EnqueueRequest, QueueJobCounts } from '@shared/queues/queue-job-store-contract';
...
export class QueueService {
    ...
    async enqueue<T extends QueuePayload>(
        ...
        if (options.preserveExistingJob && await getQueueJobStore().isJobLive(queueName, request.jobKey)) {
            return false;
        }

        await getQueueJobStore().deleteTerminalJob(queueName, request.jobKey);

        const inserted = await getQueueJobStore().insertJob(request);
        if (inserted) {
            await getQueueNotifier().notify(queueName);
        }
    ...
    async enqueueBulk<T extends QueuePayload>(
        ...
        const inserted = await getQueueJobStore().insertJobs(queueName, requests);
    ...
    retryJobById(jobId: string): Promise<boolean> {
        return getQueueJobStore().retryFailedJobByKey(jobId);
    }

    removeJobById(jobId: string): Promise<boolean> {
        return getQueueJobStore().removeJobByKey(jobId);
    }

    getJobCounts(queueName: string): Promise<QueueJobCounts> {
        assertKnownQueue(queueName);
        return getQueueJobStore().countJobsByState(queueName);
    }
