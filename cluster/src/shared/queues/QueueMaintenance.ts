...
import { sweepExpiredDaemonState } from '@shared/persistence/DaemonStateStore';
...
export class QueueMaintenance {
    ...
    async runOnce(): Promise<void> {
        ...
        try {
            const reclaimed = await getQueueJobStore().reclaimStalledJobs();
            ...
            const [purged, swept] = await Promise.all([
                getQueueJobStore().purgeExpiredTerminalJobs(),
