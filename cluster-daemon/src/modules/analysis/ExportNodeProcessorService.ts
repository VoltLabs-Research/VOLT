import { ObjectBucketName, type AnalysisExposureDefinition, type AnalysisJobExecutionData } from '../../contracts/http';
...
export class ExportNodeProcessorService {
    constructor(
    ...
    async process(input: ExportExecutionInput): Promise<void> {
        ...
        const objectPath = this.buildObjectPath(input, exporter, exportConfig.type);

        switch (exporter) {
            case 'AtomisticExporter':
                await this.exportAtomistic(input.decodedPayload, objectPath);
                break;
            case 'MeshExporter':
                await this.exportMesh(input.decodedPayload, objectPath, options as MeshExportOptions);
                break;
            case 'DislocationExporter':
                await this.exportDislocation(input.decodedPayload, objectPath, options as DislocationExportOptions);
                break;
            case 'ChartExporter':
                await this.exportChart(input.decodedPayload, objectPath, options as unknown as ChartExportOptions);
                break;
            default:
        ...
        if (exporter !== 'ChartExporter') {
            await this.sceneArtifactRepository.upsertByObjectName(objectPath, {
    ...
    private buildObjectPath(input: ExportExecutionInput, exporter: ExporterName, type: string): string {
    ...
    private async exportAtomistic(decodedPayload: Record<string, unknown>, objectPath: string): Promise<void> {
        ...
        const atomsByType = this.normalizeAtomsByType(exportData);
        const { positions, colors, min, max } = this.buildPointCloudData(atomsByType);
        const buffer = this.nativeModuleLoader.getExporterModule().generatePointCloudGLB(positions, colors, min, max);
    ...
    private async exportMesh(decodedPayload: Record<string, unknown>, objectPath: string, options: MeshExportOptions): Promise<void> {
        const exportData = getNestedValue(decodedPayload, 'export.MeshExporter');
        console.log(Object.keys(decodedPayload))
        if (!isRecord(exportData)) {
            throw new Error('Mesh export data missing from exposure payload');
        }

        const mesh = this.normalizeMesh(exportData);
        const material = this.resolveMaterial(options.material, options.enableDoubleSided);
        const processed = this.processMesh(mesh, options.smoothIterations);
        const buffer = this.nativeModuleLoader.getExporterModule().generateMeshGLB(
            processed.positions,
            processed.normals,
            processed.indices,
            false,
    ...
    private async exportDislocation(decodedPayload: Record<string, unknown>, objectPath: string, options: DislocationExportOptions): Promise<void> {
        ...
        const atomsByType = this.normalizeDislocationAsAtoms(exportData, options.typeColors, options.colorByType);
        const { positions, colors, min, max } = this.buildPointCloudData(atomsByType);
        const buffer = this.nativeModuleLoader.getExporterModule().generatePointCloudGLB(positions, colors, min, max);
    ...
    private async exportChart(decodedPayload: Record<string, unknown>, objectPath: string, options: ChartExportOptions): Promise<void> {
        ...
        const chartData = this.extractChartData(decodedPayload, options);
        if (chartData.length === 0) {
            throw new Error('No chart data found for export node');
        }

        const chartType = this.resolveChartType(options.chartType);
        const dataset = this.buildChartDataset(chartData, chartType, options);
    ...
    private normalizeAtomsByType(value: Record<string, unknown>): AtomsGroupedByType {
    ...
    private normalizeDislocationAsAtoms(
    ...
    private buildPointCloudData(atomsByType: AtomsGroupedByType): {
        ...
        typeEntries.forEach(([typeName, atoms], typeIndex) => {
            const color = this.colorForType(typeName, typeIndex);
    ...
    private colorForType(typeName: string, typeIndex: number): [number, number, number] {
    ...
    private normalizeMesh(value: Record<string, unknown>): MeshInput {
    ...
    private processMesh(mesh: MeshInput, smoothIterations?: number): {
        positions: Float32Array;
        ...
        if (smoothIterations && smoothIterations > 0) {
            this.nativeModuleLoader.getExporterModule().taubinSmooth(positions, indices, smoothIterations);
        }
    ...
    private resolveMaterial(material: ExportMaterial | undefined, doubleSided?: boolean): ExportMaterial & { doubleSided?: boolean } {
        return {
    ...
    private extractChartData(decodedPayload: Record<string, unknown>, options: ChartExportOptions): ChartPoint[] {
    ...
    private resolveChartType(chartType: ChartExportOptions['chartType']): 'line' | 'bar' | 'scatter' {
        if (chartType === 'area') {
            return 'line';
        }
